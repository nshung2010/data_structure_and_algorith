...
class LinkedBinaryTree(BinaryTree):
    ...
    def _validate(self, p):
        """Return assoiciated node, if position is valid"""
    ...
    def _add_root(self, e):
        ...
        Rase ValueError if tree nonempty
        """
    ...
    def _attach(self, p, t1, t2):
        """ Attach tre t1 and t2 as left and right subtrees of external p."""
        node = self._validate(p)
        if not self.is_leaf(p): raise ValueError('Position must be leaf')
        if not type(self) is type(t1) is type(t2): #all 3 trees must has the same type
